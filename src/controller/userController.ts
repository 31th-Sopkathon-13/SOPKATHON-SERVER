...
const createUser = async(req : Request, res : Response)=>{
    ...
    if (!userName){
        res.status(statusCode.BAD_REQUEST).send(fail(statusCode.BAD_REQUEST,message.BAD_REQUEST));
    }
    ...
    try{
        ...
        return res.status(statusCode.CREATED).send(success(statusCode.CREATED,message.CREATE_USER_SUCCESS,createdUser));
